...
resource "kubernetes_ingress_v1" "alb_ingress" {
  metadata {
    ...
    annotations = {
      ...
      "alb.ingress.kubernetes.io/load-balancer-attributes" = "deletion_protection.enabled=true,routing.http.drop_invalid_header_fields.enabled=true,access_logs.s3.enabled=true,access_logs.s3.bucket=${var.siem_storage_s3_bucket}"
    }
  }
